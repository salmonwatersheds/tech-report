...
For **Chum CU the Porcupine River**:

The estimated spawner abundance for the Chum *Porcupine* CU represents the Fishing Branch portion of the Porcupine river, which cannot be considered as an absolute count (see [Section 5.1.1.4. Spawner Abundance](#yukon-spawner-abundance)). The catch data is in Appendix B22 and also concerns only the Fishing Branch.
...
4)  Define `run_size_adjustment` = `total_run` - `estimated_count` - `combined_catch` (which is =! 0 for year \> 2011).
...
The recruits abundance data is only available from [Connors et al.'s (2025) run-reconstruction work](https://github.com/Pacific-salmon-assess/yukon-CK-ResDoc) (in particular the dataset [brood_table_long.csv](https://github.com/Pacific-salmon-assess/yukon-CK-ResDoc/blob/main/analysis/data/generated/brood_table_long.csv)) for the Chinook CUs Big Salmon, Nordenskiold, Pelly, Middle Yukon, Stewart, White and Northern Yukon.
