...
For *Teslin* we also use an alternative time series but the latter does not represent an absolute count (cf. ???). We consequently decide to not show the count data as it is not possible to discriminate between the en route mortality and abundances in un-monitored streams (i.e., `run_size_adjustement` cannot be estimated). We howevery display `exploitation_rate` because it is assumed to be the same as the other CUs.
...
5) Define for each CU the number of fish caught:
		...
		- `cdn_catch` = `combined_catch` * `total Canadian catch` / `total combined catch`
