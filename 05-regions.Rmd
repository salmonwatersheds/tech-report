...
1)  `total combined catch` = `Total Run` - `Escapement` (data from Appendix B26: total counts across the CUs)

2)  `exploitation_rate` = `total combined catch` / `Total Run`
...
5)  Finally we define the CU-level `total_run` = `estimated_count` + `combined_catch` + `run_size_adjustment`


For **Chinook CUs in the Porcupine River**:

Harvest estimate of the Porcupine river are not available and are assume to be marginal ([Connors et al. 2023][@ConnorsEtAl2023EstimatesBiologicalReference]).

For **Chum CUs in Yukon Mainstem**:

The total US and Canadian catch data and total run size (all four CUs confounded) is available in Appendix B20. The table also contains `spawning escapement` data, but that latter differs slightly from `spawning escapement estimate` in Appendix B16, which we used to define CU-level spawner abundance (cf. section ???). For consistency, we decide to use the escapement data in Appendix B16 and not Appendix B20.
...
4)  Define `exploitation_rate` = `total combined catch` / `total run size` in Appendix B20

5)  Define for each CU the number of fish caught:
...
The estimates for spawner escapement (cf. section ???) and catch the Chum *Porcupine* CU represent the Fishing Branch and are not absolute counts. The catch data is in Appendix B22.

The total US and Canadian catch data and total run size (all four CUs confounded) is available in Appendix B22. The table also contains `spawning escapement` data, but that latter differs slightly from `spawning escapement estimate` in Appendix B15, which we used to define spawner abundance (cf. section ???). For consistency, decide to use the escapement data in Appendix B15 and not Appendix B22.
...
3)  Define catches:
    ...
    ```         
     - `us_catch` = `U.S. Fishing Branch River harvest` in Appendix B22

     - `cdn_catch` = `Canada Fishing Branch River harvest` in Appendix B22
