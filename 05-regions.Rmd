...
# Import appendix 3: biostatus details
...
```{r ykMap, fig.cap = "Map of the Yukon Region.", include=TRUE, echo = FALSE, out.width='50%', fig.align='center', fig.alt=''}
knitr::include_graphics("figures/region_yukon.png")
```


### Population Data and Analytical Methods {#yukon-pop-data}

The Yukon data landscape is unique, as most fish counts are derived from U.S. border sonar monitoring stations: the Eagle sonar and the Porcupine sonar operating just downstream of Old Crow. For Chinook, run reconstruction modelling work has been undertaken to estimate CU-level spawner and recruit abundances using sonar total counts and genetic stock identification (GSI) ([Connors et al., 2022](https://doi.org/10.1002/eap.2709)). The data is maintained through the Canadian Science Advisory Secretariat (CSAS) process and the related R code, datasets and documentation are available at [yukon-CK-ResDoc](https://github.com/Pacific-salmon-assess/yukon-CK-ResDoc). For chum, run reconstruction has not been pursued but the total counts at the Eagle sonar and associated relative CU-proportions are available for several CUs in the [Yukon River Joint Technical Committee (JTC) reports](https://www.yukonriverpanel.com/publications/yukon-river-joint-technical-committee-reports/). Finally, no data has been assembled yet for the coho CU.

Spawner survey data has been collected until now but have not been reported to NuSEDS since 2008 ([Atkinson et al., 2025](https://doi.org/10.1139/cjfas-2024-0387)); therefore, they must be obtained from alternative sources.


#### Spawner Surveys {#yukon-spawner-surveys}

**Spawner survey** data for streams in the Yukon Region were sourced principally from the Yukon River Joint Technical Committee's (JTC) 2025 report [@YukonRiverJointTechnicalCommittee2025YukonRiverSalmon] (Appendices B12 and B15), Connors et al.'s (2025) report titled [Stock status and biological and fishery consequences of alternative harvest and rebuilding actions for Yukon River Chinook salmon (Oncorhynchus tshawytscha)](https://github.com/Pacific-salmon-assess/yukon-CK-ResDoc) (in particular the dataset [trib-spwn.csv](https://github.com/Pacific-salmon-assess/yukon-CK-ResDoc/blob/main/analysis/data/raw/trib-spwn.csv)), the Pacific Salmon Commission's Technical Report No. 48 [@PestalEtAl2022ReviewAvailableAbundance]<!-- ([Pestal et al., 2022](https://www.psc.org/publications/technical-reports/technical-report-series/))-->, and the cleaned version of NuSEDS [@CarturanPeacock2024CleanedSpawnerSurvey]<!--[NuSEDS](https://open.canada.ca/data/en/dataset/c48669a3-045b-400d-b730-48aafe8c5ee6)--> ([cleaned NuSEDS](https://doi.org/10.5281/zenodo.14194638)). The following sources were also used for additional data points: JTC's reports from 1999 and 2013 [@YukonRiverJointTechnicalCommittee1999YukonRiverSalmon; @YukonRiverJointTechnicalCommittee2013YukonRiverSalmon]<!--[JTC's reports from 1999 and 2013](https://www.yukonriverpanel.com/publications/yukon-river-joint-technical-committee-reports/)-->, Yukon River Panel's Restoration & Enhancement Fund project reports [@EDI2022MoreleyRivChinookRestoPlanMon; @Sparling2003InvestigationsFishHabitats]<!--([CRE-172-21](https://airtable.com/appcOJZRUczgtPBcc/shrYLkoD0iLpJzwYW/tblQ57TntOtNTYBQs?backgroundColor=yellowLight&viewControls=on), [CRE-27-02](http://www.yukonriverpanel.com/download/80/restoration/631/cre-27-02-investigations-into-fish-habitats-of-tributaries-of-the-pelly-and-mcmillan-rivers.pdf))-->, and the Canadian Technical Report of Fisheries and Aquatic Sciences No. 1351 [@MilliganEtAl1986DistributionAbundanceChum]<!--([Milligan et al., 1986](https://waves-vagues.dfo-mpo.gc.ca/library-bibliotheque/11855.pdf))-->. For a description of the spawner survey indicator, see [Overview of Population Indicators: Spawner Surveys](#spawner-surveys).


#### Juvenile Surveys {#yukon-juvenile-surveys}

The estimated juvenile abundance data for the Chinook *Big Salmon* CU originates from a mark-recapture study conducted between 2015 and 2017 in the Big Salmon River by Fisheries and Oceans Canada for the Yukon River Panel [@FisheriesandOceansCanad2019JuvenileChinookSalmon].

<!-- source: X Drive\1_PROJECTS\1_Active\Yukon\Data & Assessments\Other_Sources/Chinook_Juvenile_BigSalmon_CRE-26-18A_Appendix8.csv -->


#### Hatchery Releases {#yukon-hatchery-releases}

Hatchery releases in the Yukon Region for all species were provided by DFO (Brock Ramshaw, Salmonid Enhancement Program). Refer to [Overview of Population Indicators: Hatchery Releases](#hatchery-releases) for details on data and analytical methods.


#### Spawner Abundance {#yukon-spawner-abundance}

**Estimated spawner abundances** for Chinook were obtained from [Connors et al.'s (2025) run-reconsruction](https://github.com/Pacific-salmon-assess/yukon-CK-ResDoc) (in particular the dataset [esc-data.csv](https://github.com/Pacific-salmon-assess/yukon-CK-ResDoc/blob/main/analysis/data/generated/esc-data.csv)) for the following CUs: *Big Salmon, Nordenskiold, Pelly, Middle Yukon, Stewart, White* and *Northern Yukon*. Although estimates are also available in this dataset for the *Upper Yukon* and *Teslin* CUs, they are unrealistically high compared with local observations. To address this, we derived CU-level spawner abundances from two spawner surveys for each of these CUs (cf., details [here](https://bookdown.org/salmonwatersheds/teslin-upper-yukon-alternatives/teslin-upper-yukon-alternatives.html)). For the *Porcupine* and *Old Crow* CUs, we used estimated counts from the Porcupine sonar, as reported by the [JTC](https://www.yukonriverpanel.com/publications/yukon-river-joint-technical-committee-reports/). Although it was not possible to partition these totals between the two CUs, the combined abundance fell below the absolute benchmark of 1,500 spawners. Accordingly, we applied the same time series to both CUs ( which results in a poor biological status).

For Chum, we used [JTC]((https://www.yukonriverpanel.com/publications/yukon-river-joint-technical-committee-reports/)) data to estimate spawner abundance for four of the seven CUs, combining total escapement estimates from the Eagle sonar with relative proportion obtained from genetic stock identification. For the *Porcupine* CU, we used the Fishing Branch estimate from the [JTC 2025 report](https://www.yukonriverpanel.com/publications/yukon-river-joint-technical-committee-reports/) (Appendix B12) as relative abundance time series.


#### Run Timing {#yukon-run-timing}

Run timing data were compiled from a variety of sources detailed in @WilsonPeacock2025FreshwaterLifecycleTiming, including from the Federal Science Library (FSL) and the Alaska Department of Fish and Game (ADF&G). See [Overview of Population Indicators: Run Timing](#run-timing) for a description of the run timing indicator and methods.


#### Catch & Run Size {#yukon-catch-run-size}

All the data used comes from the [JTC](https://www.yukonriverpanel.com/publications/yukon-river-joint-technical-committee-reports/) report 2025. The nature of the data and the related procedure differ among species and groups of CUs associated to the mainstem of the Yukon river and the Porcupine river. The following sections provide the sequential steps of these procedures for each group of CUs. 

For **Chinook CUs in Yukon Mainstem**:

We first calculate the overall all exploitation rate across the CUs, which we then apply to each CU, assuming equal vulnerability to harvest:

1)  Define `total combined catch` = `Total Run` - `Escapement` (data from Appendix B26: total counts across the CUs)

2)  Define `exploitation_rate` = `total combined catch` / `Total Run`

3)  The CU-level `combined_catch` = `estimated_count` / (1 - `exploitation_rate`) \* `exploitation_rate` (we assume equal vulnerability of the CUs to harvest)

4)  We then calculate the CU-level US catch `us_catch` and Canadian catch `cdn_catch` using `total US catch` and `total canadian catch` estimates from Appendix B18 for years \> 2000. Because their sum differ slightly from `total combined catch` calculated above, we do not use these values to calculate a rate (like for `total combined catch` and `exploitation_rate`) but instead determine the relative proportions of US *vs.* Canadian catch, which we then multiply to the CU-level `combined_catch`:

    ```         
     - `us_catch` = `combined_catch` * `total US catch` / (`total US catch` + `total canadian catch`)

     - `cdn_catch` = `combined_catch` - `us_catch`
    ```

For CU *Upper Yukon*, we use an alternative absolute escapement time series that accounts for en route mortality (see [Section 5.1.1.4. Spawner Abundance](#yukon-spawner-abundance)). But in this case the harvest rate (`exploitation_rate`) should be applied to the count at the border and not in the spawning locations so we use the estimated spawner abundance from [Connors et al.'s (2025) esc-data.csv](https://github.com/Pacific-salmon-assess/yukon-CK-ResDoc/blob/main/analysis/data/generated/esc-data.csv) for `estimated_count` and define `run_size_adjustment` as the difference between the count at the border and the count in the alternative time series.

For *Teslin* we also use an alternative time series but the latter does not represent an absolute count (see [Section 5.1.1.4. Spawner Abundance](#yukon-spawner-abundance)). We consequently decide to not show the count data as it is impossible to discriminate between the en route mortality and abundances in unmonitored streams (i.e., `run_size_adjustment` cannot be estimated). We however display `exploitation_rate` because it is assumed to be the same as for the other CUs.

5)  Finally we define for each CU `total_run` = `estimated_count` + `combined_catch` + `run_size_adjustment`


For **Chinook CUs in the Porcupine River**:

Harvest estimate of the Porcupine river are not available and are assumed to be marginal [@ConnorsEtAl2023EstimatesBiologicalReference].

For **Chum CUs in Yukon Mainstem**:

The total US and Canadian catch data and total run size across all four CUs are available in Appendix B20. The table also contains `spawning escapement` data, but the latter differs slightly from `spawning escapement estimate` in Appendix B16, which we used to define CU-level spawner abundance (see [Section 5.1.1.4. Spawner Abundance](#yukon-spawner-abundance)). For consistency, we decide to use the escapement data from Appendix B16 and not Appendix B20.

As previously, we determine the overall exploitation rate (`exploitation_rate`) and we assume that all four CUs are equality vulnerable to harvest:

1)  Define `total spawning escapement` = `spawning escapement estimate` in Appendix B16

2)  Define `total combined catch` = `total US catch` + `total Canadian catch` in Appendix B20

3)  Define `total run size` = `total spawning escapement` + `total US catch` + `total Canadian catch` (instead of using `Total estimated Canadian origin run size` in Appendix 20)

4)  Define `exploitation_rate` = `total combined catch` / `total run size` 

5)  Define catch for the US `us_catch`, Canadian `cdn_catch` and total harvest `combined_catch` for each CU:

    ```         
     - `combined_catch` = `estimated_count` / (1 - `exploitation_rate`) * `exploitation_rate`

     - `us_catch` = `combined_catch` * `total US catch` / `total combined catch`

     - `cdn_catch` = `combined_catch` - `us_catch`
    ```

6)  Define for each CU `total_run` = `estimated_count` + `combined_catch`


For **Chum CU in the Porcupine River**:

The estimated spawner abundance for the Chum *Porcupine* CU represents the Fishing Branch portion of the Porcupine river, which cannot be considered as an absolute count (see [Section 5.1.1.4. Spawner Abundance](#yukon-spawner-abundance)). The catch data is in Appendix B22 and concerns only the Fishing Branch as well.

The total US and Canadian catch data and total run size across all four CUs is available in Appendix B22. The table also contains `spawning escapement` data, but that latter differs slightly from `spawning escapement estimate` in Appendix B15, which we used to define spawner abundance (see [Section 5.1.1.4. Spawner Abundance](#yukon-spawner-abundance)). For consistency, we decide to use the escapement data in Appendix B15 and not Appendix B22:

1)  Define `estimated_count`= `escapement at Fishing Branch River` in Appendix B15

2)  Define `total_run` = `Total estimated Fishing Branch River run size` in Appendix B22

3)  Define catches (from Appendix B22):

    ```         
     - `us_catch` = `U.S. Fishing Branch River harvest`

     - `cdn_catch` = `Canada Fishing Branch River harvest`

    ```

There is no value for `us_catch` in 2011 but there is one for `cdn_catch`. So we define for this year `us_catch` = `total_run` - `estimated_count` - `cdn_catch`.

```         
    - `combined_catch` = `cdn_catch` + `us_catch` for year > 2010 and = `total_run` - `estimated_count` otherwise
```

4)  Define `run_size_adjustment` = `total_run` - `estimated_count` - `combined_catch` (which is $\neq 0$ for year \> 2011).

5)  Define `exploitation_rate` = `combined_catch` / `total_run`


#### Recruits-per-Spawner {#yukon-recruits-per-spawner}

The recruits abundance data is only available for the Chinook CUs *Big Salmon, Nordenskiold, Pelly, Middle Yukon, Stewart, White* and *Northern Yukon* and it comes from [Connors et al.'s (2025) run-reconsruction work](https://github.com/Pacific-salmon-assess/yukon-CK-ResDoc) (in particular the dataset [brood_table_long.csv](https://github.com/Pacific-salmon-assess/yukon-CK-ResDoc/blob/main/analysis/data/generated/brood_table_long.csv)) .


#### Biological Status {#yukon-biological-status}

```{r, include=FALSE}
cond_rg <- app3$Region == "Yukon"
years <- unique(app3$Years[cond_rg])
years <- years[years != ""]
year <- strsplit(x = years,split = "-") |> unlist() |> max()

# NOT YUKON REGION YET
```

The **biological status** assessments for CUs in the Yukon Region are current to `r year`. As new data become available, we will update the analyses and results in this report as well as on the Pacific Salmon Explorer. See [Sections 4.1.4. Benchmarks for Assessing Biological Status](#benchmarks-biostatus) and [4.1.5. Decision Rules for Assessing Biological Status](#bs-decision-rules) for more details on biological status assessment methods.

### Habitat Data and Analytical Methods {#yukon-hab-data}

Nuances regarding habitat pressure indicator data and analytical methods for the Yukon Region are listed below. Refer to [Habitat Status: Indicators & Benchmarks](#hab-press-benchmarks) for information on habitat pressure indicators, benchmarks, and the analytical methods used to assess habitat status.

#### Assessment Watersheds

Assessment watersheds for the Yukon were sourced from the MERIT-BASINS dataset, a global watershed framework based on the MERIT Hydro hydrography product (Yamazaki et al., 2019; Lin et al., 2019). MERIT-BASINS delineates standardized watershed polygons using flow-routing algorithms applied to a globally consistent elevation model, enabling watershed-scale analyses in data-limited regions such as the Yukon.

As with other DEM-derived watershed products, MERIT-BASINS includes known limitations related to flow-routing resolution. In particular, very small watershed polygons can occur at basin outlets where multiple upstream catchments converge. These polygons reflect flow-routing resolution effects rather than discrete, independently functioning watersheds and are not suitable units for habitat pressure assessment.

To address this limitation, watershed areas were evaluated across the Yukon region and ranked by size. The distribution of watershed areas showed a clear inflection separating a small group of exceptionally small polygons from the remaining watersheds. Watersheds within this smallest 3% were considered non-representative units and were classified as Data Deficient.

#### Spawning Zones of Influence {#yukon-zoi}

Methods for delineating spawning zones of influence (ZOIs) vary by region in accordance with the CU delineation approach used by DFO. For the Yukon, spawning ZOIs for each species CU were delineated using the extent of all Assessment Watersheds that directly intersected with the CU boundaries of that species.

#### Mining Activities in the Yukon

Given the Region’s long mining history, mining-related disturbances pose significant risks to salmon habitat. Placer mining is of particular concern because operations occur directly within rivers and along their banks, displacing large volumes of sediment and water during gold extraction.

To evaluate mining pressures on salmon habitat, PSF analysts reviewed all available spatial data layers and assessed their relevance to mining disturbance. Staking and survey claim information was determined to reflect early, pre-extraction phases that cause little or no detectable landscape impact, indicating only potential future activity.

In contrast, land-use permits, mining licenses, and leases represent active surface disturbances from excavation. These disturbances can be readily identified using satellite imagery from GeoYukon’s new change-detection layer—Surface Disturbance Features—which maps areal surface impacts across the landscape. Through imagery analysis and cross-validation with other datasets, Environment Yukon analysts were able to distinguish disturbance types associated with major industries and their key activities.

In the Yukon, several spatial data layers uniquely capture both historical and current placer mining activity—unlike in British Columbia, where no publicly available placer-mining disturbance layer exists. Because placer mining is widespread across the Region and directly affects salmon habitat, these Yukon datasets were prioritized in the analysis and integrated with available information on mineral, coal, and gravel mining.

Historical placer mining information provided by the [Yukon Geological Survey](https://data.geology.gov.yk.ca/) referenced two datasets: Historical Placer Claims 1M and Historical Placer Dredging Extents. Since the 1M layer is display-only, analysts instead used the Historical Mineral Claims 50K dataset and filtered for Expired Prospecting Placer Leases, which closely aligned with the historical features shown in the Geological Yukon 1M data.

For current activity, the Current Surface Disturbance Layer—filtered to include only Significant Placer Mining Areas and General Mining Areas—corresponded well with areas captured in the Historical Placer Dredging Extents.

Together, these datasets provided a consistent and repeatable approach for identifying both historical and current placer mining disturbances across the Region.

See [Limitations: Habitat Pressure Indicators & Analysis Methods](#hab-method-limits) for special considerations when interpreting the results of other indicators in the Yukon.

#### Spawning Habitat Information from Expert Knowledge Holders

In addition to the spawning habitat information we acquired from the Yukon Fisheries Information Summary System (FISS) database, we collaborated with representatives from the Yukon First Nations Salmon Stewardship Alliance, Fisheries and Oceans Canada's staff, and salmon biologist Al von Finster, to identify local salmon spawning locations in the Region. For Chinook, we also relied on the comprehensive catalogue of Chinook salmon spawning areas by Brown et. al (2017). Efforts to expand this information will continue, to ensure that our assessments reflect the known extent of spawning habitats throughout the Yukon.
...
```{r tbrMap, fig.cap = "Map of the Northern Transboundary Region.", include=TRUE, echo = FALSE, out.width='70%', fig.align='center', fig.alt=''}
knitr::include_graphics("figures/region_northerntransboundary.png")
```
...
knitr::include_graphics("figures/region_haidagwaii.png")
```
...
**Spawner survey** data for streams in the Haida Gwaii Region were sourced from [NuSEDS](https://open.canada.ca/data/en/dataset/c48669a3-045b-400d-b730-48aafe8c5ee6). For the "North Haida Gwaii" Chinook, spawner survey data in the Yakoun River the most recent years (2021 to 2025) comes from recent monitoring done by the Haida Nation (Mark Spoljaric, personal communication, April 9, 2026). For a description of the spawner survey indicator, see [Overview of Population Indicators: Spawner Surveys](#spawner-surveys).
...
knitr::include_graphics("figures/region_nass.png")
```
...
Run timing data were compiled from a variety of sources detailed in @WilsonPeacock2025FreshwaterLifecycleTiming, including the Gitwinksihlkw fish wheel and Coded-wire Tag recovery projects at Zolzap Creek and Kincolith River. <!-- Add more details when time to insert references from Wilson and Peacocj into zotero!-->
...
knitr::include_graphics("figures/region_skeena.png")
```
...
In contrast to Chinook, there has been more consistent distribution and quantity of monitoring effort for Skeena sockeye CUs dating back to 1960. Skeena sockeye data underwent a significant review and update in 2022 [@FisheriesandOceansCanada2023BiologicalBenchmarksBuilding, @PestalCarr-Harris2025BiologicalBenchmarksBuilding]. We were provided an update of the estimated spawner abundance data file by DFO (Charmain Carr-Harris, personal communication).
...
Catch and run size, and subsequent exploitation rates for CUs in the Skeena region were calculated by LGL and DFO from the [Fisheries Operating System (FOS)](https://open.canada.ca/data/en/dataset/7ac5fe02-308d-4fff-b805-80194f8ddeb4) and other DFO databases. We accessed these datasets for pink, chum and coho salmon via the North and Central Coast (NCC) Database, maintained by LGL Ltd. [@EnglishEtAl2018NorthCentralCoast] and for sockeye following @FisheriesandOceansCanada2023BiologicalBenchmarksBuilding. For a description of catch and run size methods, see [Overview of Population Indicators: Catch & Run Size](#catch-run-size).

#### Recruits-per-Spawner {#skeena-recruits-per-spawner}

Recruits-per-spawner data for CUs in the Skeena Region were derived from the DFO age database and CU-level estimates of spawner abundance and catch and run size, accessed from the NCC Database [@EnglishEtAl2018NorthCentralCoast] for pink, chum, and coho salmon. Sockeye salmon data was from @FisheriesandOceansCanada2023BiologicalBenchmarksBuilding and Chinook salmon data was from @WintherEtAl2024AssessmentSkeenaRiver. For a description of recruits-per-spawner methods, see [Overview of Population Indicators: Recruits-per-Spawner](#recruits-per-spawner).
...
knitr::include_graphics("figures/region_centralcoast.png")
```
...
knitr::include_graphics("figures/region_fraser.png")
```
...
knitr::include_graphics("figures/region_EVIMI.png")
```
...
knitr::include_graphics("figures/region_WVI.png")
```
...
knitr::include_graphics("figures/region_columbia.png")
```
